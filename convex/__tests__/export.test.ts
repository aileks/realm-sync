...
describe('export', () => {
  ...
  describe('getExportFilename', () => {
    it('returns null when not authenticated', async () => {
      ...
      const result = await t.query(api.export.getExportFilename, { projectId, format: 'json' });
      expect(result).toBeNull();
    });

    it('generates correct filename for json format', async () => {
      const t = convexTest(schema, getModules());
      const { userId, asUser } = await setupAuthenticatedUser(t);
      const { projectId } = await createProjectWithData(t, userId);

      const result = await asUser.query(api.export.getExportFilename, {
        projectId,
        format: 'json',
      });

      expect(result).toMatch(/^test-world-export-\d{4}-\d{2}-\d{2}\.json$/);
    });

    it('generates correct filename for markdown format', async () => {
      const t = convexTest(schema, getModules());
      const { userId, asUser } = await setupAuthenticatedUser(t);
      const { projectId } = await createProjectWithData(t, userId);
      ...
      expect(result).toMatch(/^test-world-export-\d{4}-\d{2}-\d{2}\.md$/);
    });

    it('generates correct filename for csv format', async () => {
      const t = convexTest(schema, getModules());
      const { userId, asUser } = await setupAuthenticatedUser(t);
      const { projectId } = await createProjectWithData(t, userId);

      const result = await asUser.query(api.export.getExportFilename, { projectId, format: 'csv' });

      expect(result).toMatch(/^test-world-export-\d{4}-\d{2}-\d{2}\.csv$/);
    });
    ...
    it('sanitizes project name for filename', async () => {
      ...
      expect(result).toMatch(/^my-amazing-project--export-\d{4}-\d{2}-\d{2}\.json$/);
    });
  });
});

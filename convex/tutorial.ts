...
export const seedTutorialProject = mutation({
  args: {},
  handler: async (ctx) => {
    ...
    const existingTutorial = await ctx.db
      ...
      .filter((q) => q.eq(q.field('isTutorial'), true))
      .first();

    if (existingTutorial) {
      return { projectId: existingTutorial._id, alreadyExists: true };
    }
    ...
    const doc1Id = await ctx.db.insert('documents', {
      ...
      content: `In the northern reaches of the Verdant Realm, the city of Thornhaven stands as a beacon of civilization. Sir Aldric, the aging knight commander at sixty winters old, watches over its walls with unwavering vigilance.
...
Sir Aldric remembers the old tales: when dragons flee, the Shadowbane stirs. That ancient evil was sealed beneath the mountains centuries ago by the First Queen herself, using the Emerald Crown's power.
...
But the vault has been sealed since the Great War. And the key was lost when the Last King fell.`,
    ...
    const doc3Id = await ctx.db.insert('documents', {
      ...
      content: `Lady Mira discovers an ancient tome in the Eastern Tower's forbidden section. It speaks of the Shadowbane's true nature—not a creature, but a curse placed by a betrayed court mage.
...
At the Festival of Green Leaves, celebrated on the first full moon of spring, the people of Thornhaven gather unaware of the danger. Lady Mira must choose: reveal the truth and cause panic, or find the vault key before it's too late.`,
    ...
    const forestSpiritId = await ctx.db.insert('entities', {
      ...
      type: 'character',
      description: 'A being as old as the realm that dwells in Thornwood Forest.',
...
export const hasTutorialProject = mutation({
  args: {},
  handler: async (ctx) => {
    const userId = await requireAuth(ctx);
    ...
    return { hasTutorial: !!tutorial, projectId: tutorial?._id ?? null };
  },
});
